...
if [ $# -eq 0 ]
then
  echo "Usage: sh quality_creator.sh QUALITY_parameter_file.txt"
...
ADAPTER=$( grep adapter: $PARAM_FILE | awk '{print $2}' )
...
do
  ...
  echo "sh batch_QUALITY.sh $WORKING_DIR $SRA_TABLE $TYPE $Q $ADAPTER" >> $WORKING_DIR/$EXPERIMENT/batch/quality_analysis_commands.bat
  echo "sh batch_boxplot_quality.sh $QUALITY_PARAM_FILE $TYPE $Q" >> $WORKING_DIR/$EXPERIMENT/batch/quality_boxplot_commands.bat
  i=$(($i+1))
done
